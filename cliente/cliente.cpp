...
QSqlQueryModel* cliente::mostrar()
{
    ...
    model->setQuery("SELECT nombre,numeroDocumento as 'Numero de Documento',nombres as 'Nombres',primer_apellido as 'Apellido Paterno',segundo_apellido as 'Apellido Materno',telefono as 'Telefono',movil as 'Celular' FROM Cliente,Documento WHERE idDocumento=Documento_idDocumento1");
    return model;
}
...
{
    QSqlQuery query;
    query.prepare("SELECT idCliente,direccion,sexo,fechaNacimiento,telefono2,email,nacionalidad,ruc,razonSocial,direccion2 FROM Cliente,Documento WHERE Documento_idDocumento1=? AND numeroDocumento=? AND Cliente.nombres=? AND primer_apellido=? AND segundo_apellido=? AND telefono=? AND movil=? AND idDocumento=Documento_idDocumento1");
    query.bindValue(0,pDocumento.getIdDocumento());
    query.bindValue(1,numeroDocumento);
    query.bindValue(2,nombres);
    query.bindValue(3,primerApellido);
    query.bindValue(4,segundoApellido);
    query.bindValue(5,telefono);
    query.bindValue(6,movil);
    if(query.exec())
    {
        if(query.size()!=0)
        {
